			...
			<scroll-view scroll-y="true">
				<view class="list-head">
					...
					<view class="list-head-text">
						...
						<view>
							<image src="@/static/wyy.png" mode=""></image>{{creator.nickname}}
						</view>
				...
				<view class="list-music">
					...
					<view class="list-music-item" @tap="handelToDetail(item.id)" v-for="(item,index) in tracks" :key="item.id">
						<view class="list-music-top">{{index+1}}</view>
						<view class="list-music-song">
							<view>{{item.name}}</view>
							<view>
								<text v-if="privileges[index].flag >60 && privileges[index].flag < 70 " style="color:red;border: 1px solid red;transform: scale(0.8);">独家</text>
								<text v-if="privileges[index].maxbr === 999000" style="color:red;">SQ</text>
		...
		uni.showLoading({
			...
			list(options.listId).then(res=>{
				if(res[1].statusCode === 200){
					...
					this.tracks = res[1].data.playlist.tracks
					this.privileges = res[1].data.privileges
					this.$store.commit('getListSongIds',res[1].data.playlist.trackIds)
					this.isLoading = true;
					uni.hideLoading()
				}
			})
...
<style scoped>
	...
	color: white;text-align: center;line-height: 74rpx;font-size: 28rpx;}
	...
	.list-music-song view:nth-child(1){font-size: 30rpx;color: black;width: 70vw;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;}
	...
	.list-music-song view:nth-child(2) text{font-size: 20rpx; margin-right:10rpx;}
